...
const ProfessorSim = () => {
  ...
  const getCurrentDiary = (student) => {
    // Try specific evolved diary first
    if (student.formId && evolvedDiaries[student.formId]) {
      return evolvedDiaries[student.formId]
    }
    // Try specific base diary
    const baseKey = student.archetype.toLowerCase()
    if (baseDiaries[baseKey] && baseDiaries[baseKey].length > 0) {
      return baseDiaries[baseKey]
    }
    // Fallback generic diary so it never says "no entries"
    return [
      "I’ve been thinking about food more than usual lately.",
      "My clothes are starting to feel different. Tighter in some places.",
      "I keep telling myself I’ll be more careful tomorrow… but tomorrow never really comes.",
      "It’s strange how easy it is to just keep eating when no one is watching.",
      "Part of me wonders what would happen if I just stopped fighting it."
    ]
  }
...
export default ProfessorSim  const [showFeedPopup, setShowFeedPopup] = useState(false)
  const [showObservePopup, setShowObservePopup] = useState(false)
  const [showWeighInModal, setShowWeighInModal] = useState(false)
  const [showNarrativePopup, setShowNarrativePopup] = useState(false)
  const [popupMessage, setPopupMessage] = useState("")
